...
def fetch_all_zap_data(contract_info, tx_type):
    print(f'Fetching {tx_type} for zap contract...')
    contract_address, start_block = contract_info

    # initialize request id and start block from 0 or from existing data
    request_idx = file_handlers.get_or_init_zap_request_idx(tx_type)
    if request_idx != 0:
        start_block = file_handlers.load_next_zap_block(tx_type, request_idx - 1)
...
if __name__ == '__main__':
    ...
    fetch_all_zap_data(ZAP_CONTRACT_INFO, 'withdrawals')
